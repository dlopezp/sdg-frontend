...
describe("Repository should", () => {
  ...
  it("return from localStorage when exist and its valid", async () => {
    ...
    localStorage.setItem(
      "pd-data",
      JSON.stringify({
        date: new Date(2024, 11, 30).getTime(),
    ...
    expectedRegions.forEach((expectation) =>
      expect(regions).toContainEqual(expectation),
    );
    expectedEurope.forEach((expectation) =>
      expect(europe).toContainEqual(expectation),
    );
  });
  ...
  it("makes GET request to an API when data is outdated (> 30 days)", async () => {
    ...
    mockCountries.forEach((expectation) =>
      expect(onStorage.countries).toContainEqual(expectation),
    );
  });
